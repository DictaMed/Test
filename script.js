...
class AudioRecorder {
    ...
    stopRecording() {
        if (this.mediaRecorder) {
            ...
            // Mettre à jour l'UI
            this.updateStatus('stopped', '✅ Enregistré');
            this.btnRecord.classList.add('hidden');
    ...
    getSupportedMimeType() {
        ...
        const types = [
            'audio/webm;codecs=opus',  // Meilleure qualité/compression
            'audio/webm',
...
async function sendData(mode) {
    try {
        ...
        if (response.ok) {
            ...
            // Afficher le Google Sheet si c'est le mode test
            if (mode === 'test') {
            ...
            if (confirm('Voulez-vous réinitialiser le formulaire pour un nouveau patient ?')) {
                resetForm(mode);
                AutoSave.clear();
            }
        } else {
