...
const AutoSave = {
    ...
    save() {
        try {
            // SEULEMENT les identifiants d'authentification sont sauvegardés automatiquement
            const data = {
                timestamp: Date.now(),
                auth: {
    ...
    restore() {
        try {
            ...
            // NE RESTAURER QUE les identifiants d'authentification
            if (data.auth && document.getElementById('username')) {
                Object.entries(data.auth).forEach(([key, value]) => {
                ...
                Toast.info('Vos identifiants d\'authentification ont été restaurés', 'Reprise de session', 3000);
            }
        } catch (error) {
...
function switchTab(tabId) {
    ...
    // Mettre à jour le compteur de sections après le changement d'onglet
    setTimeout(() => {
        updateSectionCount();
    }, 100);
}
